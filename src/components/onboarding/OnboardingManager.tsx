...
import { WorldIntro } from '../3d/WorldIntro';
...
export function OnboardingManager({ currentWorld = 'dev', cameraRef }: OnboardingManagerProps) {
  ...
  const { discover, discoverMovement, discoverFreeCamera } = useFeatureDiscovery();
...
export function useOnboardingActions() {
  ...
  const triggerFeatureDiscovery = (featureId: string) => {
    ...
    import('./tutorialSteps').then(() => {
      import('@/store/progression-store').then(({ FEATURE_DEFINITIONS }) => {
