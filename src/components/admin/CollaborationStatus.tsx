...
export function CollaborationStatus({
  ...
  const {
    ...
    onLockConflict: (currentLock) => {
      console.log("Content locked by:", currentLock.user.name || currentLock.user.email)
    },
  })

  const canEdit = can("posts:write") || can("projects:write")
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            {isLockedByOthers && lock?.user && (
              <Avatar className="h-6 w-6">
                <AvatarImage src={lock.user.image || undefined} />
                <AvatarFallback className="text-xs">
                  {getInitials(lock.user.name || lock.user.email)}
                </AvatarFallback>
              </Avatar>
            )}
          </div>
        ...
        {lock && (
          <div className="flex items-center justify-between p-3 rounded-lg bg-slate-50 dark:bg-slate-900">
            <div className="flex items-center gap-3">
              <Avatar>
                <AvatarImage src={lock.user.image || undefined} />
                <AvatarFallback>{getInitials(lock.user.name || lock.user.email)}</AvatarFallback>
              </Avatar>
              <div>
                <p className="text-sm font-medium">
                  {lock.user.name || lock.user.email}
                  {lock.userId === session?.user?.id && " (you)"}
                </p>
                <p className="text-xs text-slate-500 flex items-center gap-1">
                  <Clock className="h-3 w-3" />
                  Expires in {formatDistanceToNow(new Date(lock.expiresAt))}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2">
  ...
  return (
    ...
    >
      ...
      <span className="text-sm font-medium">
        {isLockedByMe ? "You are editing" : `${lock.user.name || lock.user.email} is editing`}
      </span>
      <Avatar className="h-6 w-6">
        <AvatarImage src={lock.user.image || undefined} />
        <AvatarFallback className="text-xs">{getInitials(lock.user.name || lock.user.email)}</AvatarFallback>
      </Avatar>
    </div>
  )
}
