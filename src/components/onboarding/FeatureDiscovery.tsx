...
import { useProgressionStore, FEATURE_DEFINITIONS } from '@/store/progression-store';
...
export function FeatureDiscovery({ autoDiscover }: FeatureDiscoveryProps) {
  ...
  useEffect(() => {
    if (!firstVisit) {
      pendingNotifications.forEach((featureId, index) => {
  ...
  }, [pendingNotifications, firstVisit, dismissedTooltips]);
...
export function ContextualTooltip({
  ...
  const isDiscovered = useProgressionStore((s) => s.isFeatureDiscovered(featureId));
...
export function NewBadge({ featureId, className }: NewBadgeProps) {
  ...
  const isRecentlyDiscovered = useProgressionStore((s) => {
    const feature = s.getFeature(featureId);
...
export function useFeatureDiscovery() {
  ...
  return {
    ...
    discoverFreeCamera: () => discoverFeature(FEATURE_DEFINITIONS.free_camera),
    discoverInteraction: () => discoverFeature(FEATURE_DEFINITIONS.interaction),
    discoverPortals: () => discoverFeature(FEATURE_DEFINITIONS.portals),
    discoverTerminals: () => discoverFeature(FEATURE_DEFINITIONS.terminals),
    discoverDevWorld: () => discoverFeature(FEATURE_DEFINITIONS.dev_world),
    discoverArtWorld: () => discoverFeature(FEATURE_DEFINITIONS.art_world),
  };
}
